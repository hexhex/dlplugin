	...
	for (pos = conclusion.begin(); pos != conclusion.end(); pos++) 
		{
			...
			//Update u(pos->getPredicateName(), op, PREFIX_PRED + pos->getPredicateName());
	...
	for (pos = conclusion.begin(); pos != conclusion.end(); pos++) 
		{
			...
			//Update u(pos->getPredicateName(), op, PREFIX_IN + pos->getPredicateName());
...
DLRules Default::getDLRules() 
{
	...
	else 
	{
		...
		Predicate p_r1_b(PREFIX_OUT + strid.str(), all_distinct_terms_conclusion);
		DLRule r1(p_r1_h);	
		...
		for (c_pos = conclusion.begin(); c_pos != conclusion.end(); c_pos++) 
		{
			...
			//Predicate p_ri_h(PREFIX_IN + c_pos->getPredicateName(), c_pos->getTerms());
		...
		Predicate p_r3_b2(PREFIX_OUT + strid.str(), all_distinct_terms_conclusion);
		DLRule r3(p_fail);
